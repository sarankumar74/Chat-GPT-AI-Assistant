...
export default function Auth({ onAuthSuccess }: AuthProps) {
    ...
    try {
      if (isSignUp) {
        ...
        try {
          ...
          console.error("Firestore error during sign-in:", firestoreErr);
          // Still allow sign in even if Firestore fails on first attempt
          console.warn("Continuing sign-in despite Firestore error");
        }
      }
      onAuthSuccess();
    } catch (err: any) {
      ...
      if (err.code === "auth/email-already-in-use") {
        ...
        friendlyMessage = "Database error. Please check your internet connection and try again.";
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      let friendlyMessage = "Google Sign-In failed. Using Email/Password authentication is recommended.";
      
      if (err.code === "auth/popup-blocked") {
        friendlyMessage = "Sign-in popup was blocked. Please enable popups or use the Email/Password form below.";
      } else if (err.code === "auth/cancelled-popup-request") {
        friendlyMessage = "Sign-in was cancelled. Please try again or use Email/Password authentication.";
      } else if (err.code === "auth/operation-not-supported-in-this-environment") {
        friendlyMessage = "Google Sign-In is not available in this environment. Please use Email/Password authentication below.";
      } else if (err.code === "auth/unauthorized-domain") {
        friendlyMessage = "This domain is not authorized for Google Sign-In. Please use Email/Password authentication below.";
      } else if (err.code === "auth/network-request-failed") {
        friendlyMessage = "Network error occurred. Please try Email/Password authentication or check your connection.";
      }
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFB] flex flex-col items-center justify-center p-4 relative overflow-hidden" id="auth_container">
      <div className="w-full max-w-md bg-white border border-slate-200/80 rounded-2xl p-8 shadow-sm relative z-10 animate-fade-in" id="auth_card">
        ...
        {/* Google OAuth Quick Button - Optional */}
        <button
          type="button"
          onClick={handleGoogleSignIn}
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 bg-white hover:bg-slate-50 text-slate-800 border border-slate-200 font-medium py-2.5 px-4 rounded-lg transition duration-150 active:scale-[0.98] disabled:opacity-50 shadow-sm cursor-pointer text-sm opacity-60"
          id="btn_google_signin"
          title="Google Sign-In configuration required. Use Email/Password as alternative."
        >
        ...
        <div className="relative my-4 flex items-center">
          ...
          <span className="flex-shrink mx-4 text-[10px] text-slate-400 font-bold uppercase tracking-wider">Sign in with email</span>
          <div className="flex-grow border-t border-slate-100"></div>
        </div>
        ...
        {/* Helpful note about authentication */}
        <div className="mt-6 p-3 bg-blue-50 border border-blue-100 rounded-lg text-blue-700 text-xs leading-relaxed">
          <strong>📧 Email/Password Authentication:</strong> Fully functional and recommended. Create an account or sign in using your email.
          <br />
          <br />
          <strong>🔐 Google Sign-In:</strong> Requires OAuth configuration in Firebase Console. To enable: Visit your Firebase Project → Authentication → Sign-in method → Configure Google OAuth consent screen and add authorized domains.
        </div>
      </div>
    </div>
  );
